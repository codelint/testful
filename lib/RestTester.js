...
class Tester extends events.EventEmitter {
    ...
    run(_cases, callback) {
        ...
        function wrap(t, cs){
            ...
            r.title = cs['title'];
            r.caseSort = cs['sort'] ? cs['sort'] : 999999999
            r.indexSort = sort ++;
            return r;
        }
        ...
        wraps.sort(function(a, b) {
            let l1 = a.caseSort - b.caseSort
            let l2 = a.indexSort - b.indexSort
            return l1 === 0 ? l2 : l1
        });
    ...
    _testCase(_case, callback) {
        ...
        let body = Object.prototype.toString.call(input.body) === '[object String]' ? input.body : JSON.stringify(input.body);
        let port = Number(input.port || env.port || 80)
        let protocol = _case.protocol || (port === 443 ? 'https' : 'http')
        let url = `${protocol}://${input.host || env.host}${port === 443 || port === 80 ? '' : `:${port}`}${input.path}`;

        let client = this.client || new HttpClient({
            timeout: env.timeout,
            encoding: env.encoding
        })

        client.setHeaders(input.headers)
        client.call(url, input.method, body, callback);
